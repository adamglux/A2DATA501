...
#note: adams.plot() is a custom theme made for ggplots
#this theme isn't publicly available
sample1$plots$Cooks + theme_linedraw()
sample1$plots$DFFITS + theme_void()
sample1$plots$Hadi + theme_classic()

## apply a theme to a grid of plots
#library(gridExtra)
g2 <- lapply(sample1$plots, function(p) p+adams.plot())
...
cat("difference in max Cook's vs max Hadi's = ", 
    max(sample1$influence_measures$`Cook's D`) - 
      max(sample1$influence_measures$`Hadi's Influence`))
```
